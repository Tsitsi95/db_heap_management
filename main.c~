...
int main() {
	...
	char user_name[NAME_SIZE],user_surname[SURNAME_SIZE],user_city[CITY_SIZE],file[FILE_NAME],parse_file[MAX_NAME_SIZE];
	...
	while(1){
		...
		switch(choice){
			case 1:
				...
				printf("Please insert the Name of the new Heap File (existing files with the same name will be overwrited!)\n");
				scanf(" %s",file);
				HP_CreateFile(file);
				break;
			case 2:
				...
				if(number_of_heap < NUMBER_OF_OPENED_HEAP){
					...
					fprintf(stderr,"Opened Heap Files Limit Reached!-If you want to open-manage more heap files\
			...
			case 3:
				...
				if(number_of_heap <= 0){
					fprintf(stderr,"Unable to locate opened Heap File to store the loaded parsed File!\n");
					break;
				}
				printf("Please give the name of the file you want to parse and load in the latest opened Heap File !\n");
				scanf(" %s",parse_file);
				...
				while(retur){
					...
					switch(choice2){
						...
						case 1:
							...
							scanf(" %d",&user_id);
							HP_GetAllEntries(opened_heap_files[number_of_heap-1],"id",&user_id);
							break;
						case 2:
							//Search in the records of the Heap File according the Name
							printf("Give me the Name you want to lookup for:\n");
                        		        	scanf(" %s",user_name);
                                			HP_GetAllEntries(opened_heap_files[number_of_heap-1],"name",user_name);
							break;
						case 3:
							...
							printf("Give me the Surname you want to lookup for:\n");
                                			...
                                			HP_GetAllEntries(opened_heap_files[number_of_heap-1],"surname",user_surname);
							break;
						case 4:
							//Search in the records of the Heap File according the City
							printf("Give me the City you want to lookup for:\n");
                                			scanf(" %s",user_city);
                                			HP_GetAllEntries(opened_heap_files[number_of_heap-1],"city",user_city);
							break;
						case 5:
							//Print all the records of the Heap File
							HP_GetAllEntries(opened_heap_files[number_of_heap-1],"all",NULL);
							break;
						default:
							//If user give another choice warning message
							fprintf(stderr,"Given Choice of User in Menu doesn't exist!\n");
							break;
					}
				}
				break;
			case 5:
				...
				for(i=number_of_heap-1;i<=NUMBER_OF_OPENED_HEAP-2;i++){			//Shift all the next in the Queue with one step
			...
			case 6:
				...
				if((opened_heap_files[number_of_heap-2] != -1) && (number_of_heap - 2 >= 0)){
			...
			case 7:
				...
				if((opened_heap_files[number_of_heap] != -1) && (number_of_heap < NUMBER_OF_OPENED_HEAP)){
...
void Parse_Insert(char* parse_file,int recognition){
	FILE* fp;
        char* checker;
	Record rec;
